    ...
    private DataTable BuildResultTable()
    {
        ...
        var sqlBuilder = new StringBuilder(@"
            SELECT  ntd.ApplyNo, ntd.ApplyDate, ntd.BranchCode,
                    ...
                    ntd.CustomerName, ntd.TotalAmount, ntd.Status, ntd.RateTypeCode,
                    ntd.CreateUser,
        ...
        if (!Me.IsHeadOffice)
        {
            sqlBuilder.Append(" AND (ntd.BranchCode = @MyBranch OR ntd.CreateUser = @MyEmpID) ");
        ...
        {
            sqlBuilder.Append(" AND ntd.ApplyDate >= @DateFrom ");
            parameters.Add(P("@DateFrom", dateFrom));
        }
        if (DateTime.TryParse(TxtDateTo.Text, out dateTo))
        {
            sqlBuilder.Append(" AND ntd.ApplyDate < DATEADD(d, 1, @DateTo) ");
        ...
        {
            sqlBuilder.Append(" AND ntd.CustomerName LIKE @CustomerKeyword ");
            parameters.Add(P("@CustomerKeyword", "%" + TxtCustomerKeyword.Text.Trim() + "%"));
        }

        if (!string.IsNullOrWhiteSpace(TxtApplyNoKeyword.Text))
        {
            sqlBuilder.Append(" AND ntd.ApplyNo LIKE @ApplyNoKeyword ");
            parameters.Add(P("@ApplyNoKeyword", "%" + TxtApplyNoKeyword.Text.Trim() + "%"));
        }

        sqlBuilder.Append(" ORDER BY ntd.ApplyDate DESC, ntd.ApplyNo DESC ");
