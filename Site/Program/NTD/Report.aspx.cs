...
public partial class NtdReport : Page
{
    ...
    private void BindBranchDropDown()
    {
        ...
        // 公司分行資料在 chb_pub.dbo.BRANCH（欄位 BRNO/BRNAME），別名對應程式既有欄位名稱
        var branches = Query("SELECT BRNO AS BranchCode, BRNAME AS BranchName FROM chb_pub.dbo.BRANCH ORDER BY BRNO");
    ...
    /// <summary>
    /// 彙整表 SQL：以 chb_pub.dbo.BRANCH 為左主表 LEFT JOIN，
    /// 讓沒有任何申請的分行也能列出 0 件。
    /// </summary>
    private DataTable BuildSummaryTable()
    {
        // 公司分行資料在 chb_pub.dbo.BRANCH（欄位 BRNO/BRNAME）。
        // 以 BRANCH 為主表 LEFT JOIN，讓沒有申請單的分行也能列出 0 件。
        var sqlBuilder = new StringBuilder(@"
            SELECT  branch.BRNO AS BranchCode, branch.BRNAME AS BranchName,
                    COUNT(ntd.ApplyNo)                                       AS ApplyCount,
                    ISNULL(SUM(ntd.TotalAmount), 0)                          AS TotalAmount,
                    SUM(CASE WHEN ntd.Status = 9          THEN 1 ELSE 0 END) AS DoneCount,
                    SUM(CASE WHEN ntd.Status IN (1,2,4,6) THEN 1 ELSE 0 END) AS PendingCount,
                    SUM(CASE WHEN ntd.Status = 0          THEN 1 ELSE 0 END) AS RejectCount
            FROM    chb_pub.dbo.BRANCH branch
            LEFT JOIN dbo.NTD_Main    ntd
                   ON ntd.BranchCode = branch.BRNO
                  AND ntd.ApplyDate <= @BaseDate ");
        ...
        if (int.TryParse(DdlStatus.SelectedValue, out filterStatus) && filterStatus >= 0)
        {
            ...
            sqlBuilder.Append(" AND ntd.Status = @Status ");
        ...
        {
            // 對應 BRNO 欄位（原 BranchCode）
            sqlBuilder.Append(" AND branch.BRNO = @Branch ");
            parameters.Add(P("@Branch", DdlBranch.SelectedValue));
        }
        sqlBuilder.Append(" GROUP BY branch.BRNO, branch.BRNAME ORDER BY branch.BRNO ");
    ...
    private DataTable BuildAuditTable()
    {
        ...
        var sqlBuilder = new StringBuilder(@"
            SELECT  ntd.ApplyNo, ntd.ApplyDate, ntd.BranchCode,
                    ISNULL(branch.BRNAME, '') AS BranchName,
                    ntd.CustomerName, ntd.TotalAmount, ntd.Status, ntd.CreateUser,
                    ntd.CreateUser            AS CreateUserName,
                    CASE WHEN ntd.UpdateTime IS NULL
                         THEN DATEDIFF(d, ntd.CreateTime, @BaseDate)
                         ELSE DATEDIFF(d, ntd.UpdateTime, @BaseDate) END AS StayDays
        ...
        if (!string.IsNullOrEmpty(DdlBranch.SelectedValue))
        {
            sqlBuilder.Append(" AND ntd.BranchCode = @Branch ");
            parameters.Add(P("@Branch", DdlBranch.SelectedValue));
        }
        ...
        sqlBuilder.Append(" ORDER BY StayDays DESC, ntd.ApplyNo ");
        var table = Query(sqlBuilder.ToString(), parameters.ToArray());
        NtdCurrentUser.FillEmpNames(table, "CreateUserName");
        return table;
    }
