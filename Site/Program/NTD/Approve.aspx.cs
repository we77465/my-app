...
public partial class NtdApprove : Page
{
    ...
    private void BindPendingList()
    {
        ...
        string sql = @"
            ...
            WHERE   ntd.Status = @Status ";

        var parameters = new List<SqlParameter> { P("@Status", MyHandlingStatus) };
        if (!Me.IsHeadOffice)
        {
            sql += " AND ntd.BranchCode = @MyBranch ";
            parameters.Add(P("@MyBranch", Me.BranchCode));
        }
        sql += " ORDER BY ntd.ApplyDate, ntd.ApplyNo ";

        var table = Query(sql, parameters.ToArray());
        NtdCurrentUser.FillEmpNames(table, "CreateUserName");
        GvPendingList.DataSource = table;
