...
partial class FrmSubM5
{
    ...
    private void InitializeComponent()
    {
        ...
        // 舊版：BAR4=BK505(百年規格), BAR5=BK504(舊規格)
        var mSecond = _Top("二次轉扣帳作業(&D)",
            _Item("結果磁片匯入",                   miSecImport_Click),
            _Item("失敗明細查詢",                   miSecFail_Click),
            _Item("成功明細查詢",                   miSecOk_Click),
            _Item("產生二次處理磁片(百年規格)",     miSecExpNew_Click),   // BAR4：百年（舊版順序）
            _Item("產生二次處理磁片(舊規格)",       miSecExpOld_Click));  // BAR5：舊規格（舊版順序）

        // === 密碼作業 - 對應 frmpswd，直接進入變更密碼 ===
        ...
        var mMnt = _Top("系統維護(&S)",
            _Item("銀行基本資料設定",   miSysBank_Click),
            _Item("轉帳類別設定",       miSysTr11_Click),
            _Item("企業基本資料設定",   miSysEtps_Click),
            _Item("客戶資料設定",       miSysCust_Click),
            _Item("客戶資料設定2",      miSysCus1_Click),
            new System.Windows.Forms.ToolStripSeparator(),
            _Item("對應資料設定",       miSysMat_Click),
            new System.Windows.Forms.ToolStripSeparator(),
            _Item("DBF 檔案資訊",       miDbfInfo_Click),
            _Item("DBF 資料匯入",       miDbfIn_Click));
