...
/// <summary>BK106 薪資輸入-姓名 (依員工姓名查詢)</summary>
public partial class BK106 : CrudFormBase
{
    private int _idx = -1;
    ...
    {
        var name = txtKey.Text.Trim();
        if (string.IsNullOrEmpty(name)) return;
        if (Cur != null && Cur.EMP_NAME == name) return;
        var idx = BkacRepository.Employees.FindIndex(x => x.EMP_NAME.Contains(name));
        if (idx < 0) MessageBox.Show("找不到該員工姓名", "注意");
        else BindAt(idx);
    }

    private void btnSave_Click(object s, EventArgs e)
    {
        if (Cur is not Bkac cur) { MessageBox.Show("請先輸入員工姓名查詢"); return; }
