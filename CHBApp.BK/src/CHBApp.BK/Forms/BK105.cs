...
/// <summary>BK105 薪資輸入-代號 (依員工編號查詢)</summary>
public partial class BK105 : CrudFormBase
{
    private int _idx = -1;
    ...
    private Bkac? Cur => (_idx >= 0 && _idx < BkacRepository.Employees.Count)
    ...
    {
        var no = txtKey.Text.Trim();
        if (string.IsNullOrEmpty(no)) return;
        if (Cur != null && Cur.EMP_NO == no) return;
    ...
    {
        if (Cur is not Bkac cur) { MessageBox.Show("請先輸入員工代號查詢", "注意"); return; }
    ...
    { Type = BkReportPrinter.ReportType.PayrollList, Title = "本月薪資清冊 - 個人",
      Employees = Cur != null ? new List<Bkac> { Cur } : new() };
