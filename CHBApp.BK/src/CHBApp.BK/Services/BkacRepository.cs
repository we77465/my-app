...
public static class BkacRepository
{
    ...
    /// <summary>
    /// 使用者清單：優先從 USER_INFO.DBF 讀入（密碼自動解密），
    /// 若 DBF 不可存取則退回內建測試帳號。
    /// </summary>
    public static List<AppUser> Users { get; } = LoadUsers();

    private static List<AppUser> LoadUsers()
    {
        var fromDbf = UserRepository.LoadAll();
        if (fromDbf.Count > 0) return fromDbf;

        // DBF 讀取失敗時的 fallback（測試環境用）
        return new List<AppUser>
        {
            new() { UserId = "1111",  Password = "1111",  UserName = "系統管理員", Group = "A" },
            new() { UserId = "admin", Password = "admin", UserName = "Admin",   Group = "A" }
        };
    }
