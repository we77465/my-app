<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Query.aspx.cs" Inherits="Program_KL_Query" %>
    ...
    <form id="form1" runat="server">
        <div>
                        <table border="1" class="auto-style8">
                            <tr>
                                <td class="auto-style9">分行：</td>
                                <td height="30px">
                                   <asp:DropDownList ID="drpBrno" runat="server" AppendDataBoundItems="True" DataSourceID="sdsBrno" DataTextField="BRNAME" 
                                        DataValueField="BRNO">
                                        <asp:ListItem Value="%">查詢全部</asp:ListItem>
                                </asp:DropDownList>
                                </td>
                            </tr>
                            <tr>
                     ...
                     <asp:Button ID="btnQuery" runat="server" OnClick="btnQuery_Click" Style="font-family: 微軟正黑體; font-size: medium" Text="查詢" ValidationGroup="vQTY" />
                        ...
                        <asp:GridView ID="gvData" runat="server" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False"
                            ...
                            EmptyDataText="查無資料！" GridLines="Horizontal" Style="font-size: medium; font-family: 微軟正黑體"
                            OnRowCommand="gvData_RowCommand">
                            <AlternatingRowStyle BackColor="#F7F7F7" />
                            <Columns>
                                <asp:TemplateField ShowHeader="False">
                                    <ItemTemplate>
                                        <asp:LinkButton ID="LinkView" runat="server" CausesValidation="False" CommandArgument='<%# Eval("No") %>' CommandName="Select" Text="明細"></asp:LinkButton>
                                    </ItemTemplate>
                                </asp:TemplateField>
         ...
         <asp:SqlDataSource ID="sdsList" runat="server" ConnectionString='<%$ ConnectionStrings:chb_iom %>'
            SelectCommand="select [No] ,ID ,[Type],case [Status] when '1' then '待組長簽核' when '2' then '組長核可,待處長核可' when '3' then '組長退回' when '4' then '處長核可' when '5' then '處長退回' end [StatusName] from KL where [Status] in ('1','2','3','4','5')
                         and [ID] like @ID   and Brno like @Brno">
            <SelectParameters>
                <asp:Parameter Name="ID" />
                <asp:ControlParameter ControlID="drpBrno" Name="Brno" PropertyName="SelectedValue" />
            </SelectParameters>
        </asp:SqlDataSource>
